  ...
  <div>
    ...
    <el-card shadow="never" style="margin-bottom: 16px;">
      <el-row :gutter="16" style="align-items: center;">
        ...
        <el-col :span="4">
          <el-select v-model="enabledFilter" placeholder="状态筛选" clearable @change="fetchRiders">
        ...
        <el-col :span="4">
          <el-button type="primary" @click="fetchRiders">
    ...
    <el-card shadow="never">
      <el-table :data="riderList" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column prop="completedOrders" label="完成订单" width="110" align="center">
          <template #default="{ row }">{{ row.completedOrders || 0 }}</template>
        </el-table-column>
      ...
      <div style="display: flex; justify-content: flex-end; margin-top: 16px;">
        <el-pagination
          ...
          @size-change="fetchRiders"
          @current-change="fetchRiders"
        />
      </div>
    </el-card>
  </div>
...
async function fetchRiders() {
  ...
  try {
    ...
    const { data } = await request.get('/api/admin/riders', { params })
    if (data.content) {
      riderList.value = data.content
      total.value = data.totalElements || 0
    } else {
      riderList.value = Array.isArray(data) ? data : []
      total.value = riderList.value.length
    }
  } catch (e) {
    riderList.value = []
  } finally {
...
async function handleToggle(row) {
  ...
  try {
    await request.put(`/api/admin/riders/${row.id}/status`, { enabled: newEnabled })
